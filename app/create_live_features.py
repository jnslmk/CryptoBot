...
def append_df_to_csv(df, csv_file_path, sep=","):
    ...
    if not os.path.isfile(csv_file_path):
        df.to_csv(csv_file_path, mode='a', index=True, sep=sep)
    else:
        df.to_csv(csv_file_path, mode='a', index=True, sep=sep, header=False)
...
def get_latest_book_timestamp():
    book = next(books_db.find({}, {'_id': 1}).sort('_id', -1).limit(1))
    return book['_id']
...
while True:
    ...
    if this_data_timestamp < (start - 3):
        print("Data hasn't been updated in less than 3 seconds, "
              "skipping...", this_data_timestamp, start - 3)
    elif last_data_timestamp != 0 and last_data_timestamp == \
            this_data_timestamp:
        print("Last data timestamp is equal to this data timestamp, "
              "skipping...")
    else:
