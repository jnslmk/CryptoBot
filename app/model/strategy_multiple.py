...
def get_formatted_time_string(this_time):
    return datetime.datetime.utcfromtimestamp(this_time).strftime(
        timestamp_format)
...
def fit_and_trade(data, cols, threshold, threshold_percent):
    ...
    print('Test data starts at %s (%s), ends at %s (%s)' % (
        get_formatted_time_string(X.index.values[0]), X.index.values[0],
        get_formatted_time_string(X.index.values[-1]), X.index.values[-1]))
...
def trade(X, y, index, threshold, threshold_percent):
    ...
    preds = {}
    for prediction_period in prediction_periods:
        preds[prediction_period] = models[prediction_period].predict(
            X) / y_multiplied_by
    ...
    for i in range(len(index)):
        if active:
            if (index_as_dates[
                    i] - trade_at).total_seconds() >= prediction_duration:
                trade_at = 0
                active = False
            ...
            print_string = "%s %s Trading %s at predictions: " % (
            i, index[i], trades[i])
            for key in preds:
                print_string = print_string + "%s=%s " % (
                key, format(preds[key][i], '.10f'))
            print(print_string)
            print_string = "%s %s                 Real mids: " % (i, index[i])
            for key in preds:
                print_string = print_string + "%s=%s " % (
                key, format(y[key][i], '.10f'))
            print(print_string)
            print("")
    ...
    for i in range(len(trades)):
        if trades[i] != 0 and returns[i] > 0:
            ...
            for x in prediction_periods:
                for check_timestamp_index in range(i, len(index)):
                    if index[check_timestamp_index] - this_timestamp >= x:
                        print("For trade made at timestamp", this_timestamp,
                              "period of", x, "is found at",
                              index[check_timestamp_index], "with prediction=",
                              format(preds[x][check_timestamp_index], '.10f'),
                              "real=",
                              format(y[x][check_timestamp_index], '.10f'))
                        if (np.sign(y[x][check_timestamp_index]) != np.sign(
                                preds[x][check_timestamp_index])):
            ...
            if not all_predictions_correct:
                print(
                    "***A prediction for this trade was not correct, reversing return")
                returns[i] = (returns[i] * -1.0)
            else:
                print(
                    "+++All predictions were correct! This is a definite positive return")
